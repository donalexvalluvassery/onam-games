  ...
  $.fn.crossword = function (entryData) {
    ...
    var puzInit = {
      ...
      checkAnswer: function (e) {
        ...
        valToCheck = puzz.data[activePosition];
        search_key = {
          startx:
            valToCheck.startx -
            (valToCheck.orientation === "across"
              ? valToCheck.answer.length
              : 0),
          starty:
            valToCheck.starty -
            (valToCheck.orientation === "down" ? valToCheck.answer.length : 0),
          orientation: valToCheck.orientation,
        };
        ...
        var match = false;
        $.ajax({
          url: siteDomain + "/validatecrosswordentry",
          type: "POST",
          data: JSON.stringify({ value: currVal, key: search_key, user: user }),
          ...
          success: function (data) {
            if (data.match) {
              ...
              if (data.levelComplete) {
                location.replace(`${location.protocol}/?user=${user}`);
              }
